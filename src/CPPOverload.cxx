...
// look for known signatures ...
    ...
    if (!meth && (ctxt.fFlags & CallContext::kIsConstructor)) {
      ...
      // only prepend self and retry when there are real value args to forward to a
      // generated/forwarding ctor. with no value args this is default construction:
      // prepending self would then spuriously match the implicit copy ctor (self binds
      // const T&) instead of failing, which is what the base-second dispatcher hit.
    ...
    // Subscript tie-break: cppyy maps both operator[] and operator() into the
    ...
    // signatures, which Clang then reports as ambiguous -- yet in C++ `x[i]` binds
    // to operator[] alone and never considers operator(). Mirror that: if the
    // ambiguity for a subscript method contains exactly one operator[], prefer it.
    if (!meth && !ambiguous_candidates.empty() &&
        (pymeth->fMethodInfo->fName == "__getitem__" ||
         pymeth->fMethodInfo->fName == "__setitem__")) {
      Cppyy::TCppMethod_t subscript = nullptr;
      bool unique = true;
      for (auto cand : ambiguous_candidates) {
        const std::string nm = Cppyy::GetScopedFinalName(cand.data);
        if (nm.size() >= 10 && nm.compare(nm.size() - 10, 10, "operator[]") == 0) {
          if (subscript) { unique = false; break; }
          subscript = cand;
        }
      }
      if (subscript && unique) {
    ...
    for (const auto i : overloads) {
        overload_signatures << "  " << Cppyy::GetMethodReturnTypeAsString(i) // TODO: mention if the function is static from the list
                            << " " << Cppyy::GetScopedFinalName(i.data)
